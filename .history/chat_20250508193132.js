...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initializeSupabase() {
        try {
            if (win.UNDERWEB && win.UNDERWEB.common && win.UNDERWEB.common.initSupabase) {
                ...
                console.log('Using common.js to initialize Supabase');
                supabaseClient = await win.UNDERWEB.common.initSupabase();
            } else if (win.supabase) {
                // Fallback if common.js is not loaded
    ...
    // Initialize Supabase and set demo user if it fails
    initializeSupabase().then(success => {
        if (!success) {
            // In demo mode or in case of error, set a mock user
            setDemoUser();
        }
    });
